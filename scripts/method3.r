...
#CAN NOT USE MCLAPPLY BECAUSE WINDOWS DOES NOT SUPPORT :(
#Do it like this instead:

MTweedieTests <- function(N, M, sig) {
  cl <- makeCluster(detectCores()) # Create a cluster with the specified number of cores
  clusterEvalQ(cl, library(tweedie)) #We need to learn the "workers" what tweedie is
  clusterExport(cl,list('simTweedieTest','df', "i")) #also inform variables
